...
function buildSelector(options: ComponentOptions, projectPrefix: string) {
  ...
  //throw new SchematicsException(`Sel "${selector}" `);
  if (options.prefix) {
...
export default function (options: ComponentOptions): Rule {
  return async (host: Tree) => {
    //throw new SchematicsException(`Sel "${strings.dasherize(options.name)}" `);
    ...
    const skipStyleFile = options.inlineStyle || options.style === "none";//Style.None;
